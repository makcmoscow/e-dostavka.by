...
import datetime
import os
url = 'https://e-dostavka.by/catalog/item_633271.html'
...
class Parser:
    ...
    def get_name(self):
        self.name = self.driver.find('h1').text

    def get_categories_trees(self):
        self.tree_values = self.driver.find(class_ = 'breadcrumbs').find_all('a')
    ...
    def get_photo(self):
        self.photo = self.driver.find(class_ = 'increaseImage').get('href')
    ...
    def get_description(self): # Article, barcode, country, trademark, weight
        description_table = self.driver.find(class_ = 'description').find_all('li')
        description_dict = {}
        for li in description_table:
            description_dict[li.find('strong').text] = li.find('span').text
        try:
    ...
    def get_property(self):
        try:
            property_table = self.driver.find(class_ = 'property_group_9').find_all('tr')
            # property_dict = {}
    ...
    def get_about(self):
        try:
            self.about = self.driver.find(class_ = 'property_3220').text
            print(self.about)
        except AttributeError:
            try:
                self.about = self.driver.find(class_ = 'property_3221').text
                print(self.about)
            except AttributeError:
                pass
    ...
    #     time.sleep(0.5)
    #     self.driver.find('li', class_ = 'ui-state-default ui-corner-top').click()
            ...
            if url in url_list:
                ...
                try:
                    ...
                    if parser.is_remove:
                        ...
                        print('saved')
                        os.startfile(
                            r'D:\python\e-dostavka.by\requests_parser_pages.bat')
                        parser.driver.quit()
                        quit()
        parser.workbook.save(path + parser.excel_filename)
        print('saved2')
